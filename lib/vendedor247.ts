...
function systemPrompt(
  ...
  return [
    ...
    "- Al pedir la dirección, aclará SIEMPRE cómo mandarla: \"escribime la dirección (calle y número, y alguna referencia) o compartime tu ubicación desde el clip de WhatsApp\". Si el cliente manda un enlace de mapa, ESO ES la dirección: dala por recibida y no la vuelvas a pedir.",
    ...
    "- COBRO POR QR: en el MISMO mensaje en que tomás el pedido, agregá también el marcador EXACTO [QR:MONTO] con el total en número, por ejemplo [QR:600]. El sistema genera y ENVÍA el QR del banco automáticamente, así el cliente puede pagar en el momento. NO preguntes otra vez la forma de pago antes de mandarlo: mandá el QR y aclarale que si prefiere pagar en efectivo contra entrega también puede. La ÚNICA excepción es que el cliente ya haya dicho que paga en efectivo: ahí no pongas el marcador del QR. Nunca menciones el marcador ni lo expliques. Una sola vez por pedido.",
