...
/**
 * Animación de inicio de la web. Depende del rubro (lib/rubros.ts):
 *  · "petals" (florería): una rosa cuyos pétalos brotan y luego se desvanece.
 *  · "mark"  (resto):     el isotipo del rubro apareciendo sobre el color de marca.
 *
 * Es 100% CSS (keyframes con fill-mode), así que nunca se queda colgada aunque
 * el JS no llegue a ejecutarse; el timer solo la desmonta del DOM al terminar.
 */
export function RoseIntro({ enabled }: { enabled: boolean }) {
  ...
  const petals = business.rubro.intro === "petals";
  const { accent, accentDeep } = business.colors;
    ...
    >
      <div className="pop-in flex flex-col items-center">
        {petals ? (
          ...
          <div
            className="grid h-[150px] w-[150px] place-items-center rounded-[38px] text-white shadow-cardHover"
            style={{
              background: `linear-gradient(160deg,${accent},${accentDeep})`,
              animation: "pop-in 620ms cubic-bezier(0.34,1.56,0.64,1) both",
            }}
          >
            <Icon name={business.rubro.icon} size={72} />
          </div>
        )}

        <div className={`text-center ${petals ? "-mt-1" : "mt-5"}`}>
          <p className="text-[1.7rem] font-semibold leading-none tracking-[-0.5px] text-ink">
            {business.nameLight && business.name.startsWith(business.nameLight) ? (
              <>
                <span className="font-light">{business.nameLight}</span>
                {business.name.slice(business.nameLight.length)}
              </>
            ) : (
              business.name
            )}
